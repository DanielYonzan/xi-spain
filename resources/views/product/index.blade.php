...
@section('content')
    <div class="page">
        ...
        <div class="row">
            ...
            <div class="col-6 form-group">
                <div class="table-tools text-right"></div>
            </div>
        </div>
        <table class="table table-bordered advanced-table" style="width:100%">
            ...
            <tr>
                ...
                <th>Cases per palet</th>
                <th>Cases per palet layer</th>
                <th>palets per 20 container</th>
                <th>self life</th>
                <th>Origin</th>
                <th>Min Order</th>
                <th>Description</th>
            ...
            <tr>
                <td>{{$i++}}</td>
                <td>{{$d->$name_key}}</td>
                <td><img class="img-fluid" src={{asset('img/product/'.$d->image)}} width="100" alt="{{$d->name_en}}"></td>
                <td>{{$d->size.' '.$d->unit}}</td>
                <td>{{$d->price}}</td>
                <td>{{$d->units_per_case}}</td>
                <td>{{$d->cases_per_palet}}</td>
                <td>{{$d->cases_per_palet_layer}}</td>
                <td>{{$d->palets_per_20_container}}</td>
                <td>{{$d->self_life}}</td>
                <td>{{$d->origin}}</td>
                <td>{{$d->min_order}}</td>
                <td>{{$d->$short_description_key}}</td>
                <td>
                    @if($d->status)
...
@section('extra-scripts')
    ...
    <script>
        ...
        var loader = '<i class="fa fa-circle-o-notch fa-spin fa-2x fa-fw"></i><span class="sr-only">Loading...</span>';
        ...
        $('select[name="status"]').change(function(){
            $('.advanced-table tbody').html('<td align="center" colspan="10">'+loader+'</td>');
            var status = $(this).val();
            $.ajax({
                type:'post',
                url:'/admin/filterProducts',
                data:{'_method':'post','_token':'<?php echo csrf_token() ?>','status':status},
                success:function(response) {
                    if(table){
                        table.fnDestroy();
                    }
                    $('.advanced-table tbody').html(response.html);
        ...
        function initalizeDataTable(){
            table = $('.advanced-table').dataTable( {
                "responsive": true,
                "order": [],
                "columnDefs": [ {
                    "targets"  : [0],
                ...
                new $.fn.dataTable.Buttons(table, {
                    buttons: [{
                        ...
                        customize: function (doc) {
                            ...
                            doc.styles.tableHeader.fillColor = '#fed136';
                        },
                        exportOptions: {
                            stripHtml: false,
