            ...
            <table class="table table-bordered advanced-table" style="width:100%">
                ...
                @foreach($data as $d)
                    <tr>
                        <td>{{$i++}}</td>
                        <td>{{$d->name_en}}</td>
                        <td><img class="img-fluid" src={{asset('img/feature/'.$d->image)}} width="100" alt="{{$d->name}}"></td>
                        <td>{{$d->rank}}</td>
                        <td>
                            @if($d->status)
                        ...
                        <td>
                            <a href="{{route('feature.edit',$d->id)}}" class="btn btn-sm btn-success">Edit</a>
                            <form class="d-inline-block" action="{{route('feature.destroy',$d->id)}}" method="post">
                                {{csrf_field()}}
                                {{method_field('delete')}}
